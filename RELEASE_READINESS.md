...
마지막 실측: **2026-08-04 (TASK-5301 · Windows 로컬 환경)**

> ⚠️ **환경이 바뀌었습니다.** 이 문서의 아래 절들은 Linux 컨테이너에서
> 측정된 것이고, 지금 실행 환경은 Windows 로컬 머신입니다. **두 환경의
> 값을 섞어 읽지 마십시오.** 현재 환경의 실측값은 바로 다음 절에 있습니다.

---

## 2026-08-04 실측 — 그리고 새로 확인된 최상위 저지 항목

### 🔴 development 등급에서는 v1.0을 붙일 수 없습니다 (구조적)
...
즉 아래 "정식 `v1.0.0`을 붙이는 조건" 4번(`cutover` 3/3)은 **자격 증명과
무관하게** 이 머신에서 충족 불가입니다. **출시 호스트를 먼저 정하는 것이
자격 증명 주입보다 앞섭니다.**
...
0. **출시 환경이 production 또는 staging 등급이다** ← **2026-08-04 추가**
1. ① 넷이 준비된다
2. ② `validation_runs`에 **통과 기록이 남는다** (0건이 아니게 된다)
3. ③ 운영 저장소가 Amazon S3가 된다
4. `pnpm cutover`가 **운영 활성화 3/3**을 낸다

**0번이 1번보다 앞섭니다.** 0번이 아니면 4번이 구조적으로 불가능하고,
환경이 정해지지 않으면 1번의 자격 증명을 어디에 넣을지도 정해지지 않습니다.
0번은 **코드로 만들 수 없고 사람이 호스트를 주어야** 합니다.

다섯이 충족되면 `RELEASE_NOTES_v1.0.md`를 정식판으로 고치고 `v1.0.0` 태그를
붙입니다. **그 전까지는 Release Candidate입니다.**

> 참고 — `v1.0.0-rc.1` 태그는 **이 저장소의 로컬에도 없습니다**
> (`refs/tags` 비어 있음). 위 §"먼저"의 명령이 아직 수행되지 않은 상태입니다.
