...
TASK-1601이 "백업이 있고 복원된다"를 증명했다면, 이번은 그 다음 세 질문이다.
실행 절차는 [재해 복구 문서](../operations/disaster-recovery.md), 백업 방식
검토는 [backup-strategy.md](../operations/backup-strategy.md)에 있다.

### 1. 덤프가 온전한가 — Integrity

백업 직후 `pg_restore --list`로 목차를 읽고 SHA-256을 기록한다.

- **복원하지 않고도 파일이 온전한지 알 수 있는 가장 싼 검사**다. 읽히지 않는
  덤프는 존재하지만 쓸 수 없는 파일이고, 그것을 "백업 있음"으로 세면 복구
  계획이 거짓이 된다.
- 목차가 비어 있으면 읽힌 것이 아니다 — 성공으로 세지 않는다.
- 무결성 실패는 **복구 가능성을 좌우하는 항목**(critical)이다.
- 체크섬은 이력에 그대로 남긴다 — 원격 사본과 대조할 근거이기 때문이다.
  다만 체크리스트 설명에는 앞 12자만 싣는다(읽으라고 있는 문장이므로).

### 2. 같은 곳에만 있지 않은가 — Offsite Replication

`BACKUP_OFFSITE=on`이면 덤프를 오브젝트 저장소에도 올린다.

- 백업이 데이터베이스와 **같은 곳에만** 있으면, 그 곳이 사라질 때 백업도
  사라진다.
- **복제 실패가 백업을 실패시키지는 않는다** — 덤프는 이미 받았다. 대신
  로그로 남기고 화면에서 드러낸다.
- 미구성은 실패가 아니라 `warn`이고, 복구 가능성을 막지도 않는다 — 로컬
  덤프로 복구 자체는 되기 때문이다.
- 원격 키는 API에 노출하지 않는다(있는지만 알린다) — 저장소 구조를 드러낼
  이유가 없다.

### 3. 얼마를 잃고 얼마나 걸리는가 — RPO · RTO

| | 정의 | 기본 목표 |
| --- | --- | --- |
| RPO | 마지막 성공 백업 이후 경과 | `BACKUP_RPO_HOURS` (24시간) |
| RTO | **마지막 복원 검증에 실제로 걸린 시간** | `BACKUP_RTO_MINUTES` (30분) |

RTO는 추정이 아니라 **측정치**다 — 이미 복원 검증이 매일 돌고 그 소요 시간을
기록하고 있으므로, 별도 계측 없이 근거 있는 값을 쓴다.

다만 이는 **하한**이다: 사람이 장애를 알아차리고 결정하는 시간, 인프라를
준비하는 시간은 포함하지 않는다. 화면에도 그렇게 적는다. 측정치가 없으면
`manual`이고 **통과가 아니다**.

### 4. 복원 대상 강제 분리 (CTO 결정 1601-②)

복원은 대상 스키마를 **지우고 쓴다**. 대상이 운영 DB면 검증이 곧 사고다.

- 호스트·포트·DB 이름만 비교한다 — **자격 증명이 달라도 같은 DB는 같은 DB**다.
- 같으면 **기동하지 않는다**(환경 검증 오류). 개발 환경에서도 오류다 —
  운영 DB를 지우는 설정은 어디서도 허용할 수 없다.
- 기동 후 설정이 바뀔 수 있으므로 **실행 직전에 한 번 더** 본다.

### 5. 이미지 저장소 보호 상태 (CTO 결정 1601-④)

**애플리케이션은 이미지를 직접 백업하지 않는다.** 저장소 제공자의 버전 관리·
복제에 의존하되, 그 **상태를 체크리스트에 표시**한다.

- 저장소가 알려 주지 않으면 `manual` — 조회를 지원하지 않는 저장소를
  "정상"이라고 말하면 그 화면은 거짓이 된다.
- 꺼져 있으면 `warn`이지 `fail`이 아니다. 버전 관리는 삭제·덮어쓰기로부터
  보호하는 장치이지, 지금 이미지가 없다는 뜻이 아니다.

### 6. BACKUP_DIR 운영 필수 (CTO 결정 1601-①)

미설정이면 **기동하지 않는다**(Fail Fast). 백업이 컨테이너와 함께 사라지는
구성으로 운영을 시작할 수는 없다.

### 7. WAL·PITR·Incremental (CTO 결정 1601-③)

이번 Sprint는 **도입 여부 검토**만 했다 — 결론과 근거는
[backup-strategy.md](../operations/backup-strategy.md)에 있고, 요약하면
현재 데이터 규모에서는 **백업 주기 단축이 PITR보다 비용 대비 효과가 크다**는
권고다. 판단 기준은 전부 이미 기록 중인 값(`backup_runs`)으로 확인할 수 있게
두었다.
...
| 변수 | 뜻 | 기본값 |
| --- | --- | --- |
| `BACKUP_OFFSITE` | 원격 복제 (`on`/`off`) | `off` |
| `BACKUP_OFFSITE_PREFIX` | 원격 키 접두사 | `backups/` |
| `BACKUP_RPO_HOURS` | 허용 손실 구간 | 24 |
| `BACKUP_RTO_MINUTES` | 복원 허용 시간 | 30 |
| `BACKUP_MAX_AGE_HOURS` | 백업 신선도 한계 | 48 (결정 1601-⑤) |
| `RESTORE_MAX_AGE_HOURS` | 복원 검증 신선도 한계 | 192 (결정 1601-⑤) |

`GET /ops/readiness`에 `enterprise` 항목이 추가됐다 —
`integrity` · `offsite` · `storageProtection` · `objectives` · `restoreTarget`.

---

## Enterprise Operations Platform (TASK-1801, Sprint 18)

CTO 결정 1701-①~⑤를 반영한다. 큰 줄기는 **"판정만 하던 것을 지키게 만드는"**
일이다 — 손실 구간을 실제로 줄이고, 백업을 이미지와 분리하고, 리허설을
기록 대상으로 만든다.

### 1. 백업 주기 1시간 (CTO 결정 1701-①)

`backup`이 시각 기반에서 **간격 기반**으로 바뀌었다
(`OPS_CHECK_BACKUP_INTERVAL`, 기본 `1h`).

- RPO 상한이 **24시간 → 2시간**이 됐다. 복구 절차는 전혀 바뀌지 않았다 —
  `backup-strategy.md`가 PITR보다 이쪽을 먼저 권고한 이유다.
- **손실 한도 목표가 간격을 따라간다** — `defaultRpoTargetMs(간격) = 간격 × 2`.
  간격만 바꾸고 목표를 24시간에 묶어 두면 백업이 20시간 멈춰도 "정상"으로
  보인다. 설정과 판정이 어긋나면 화면이 거짓이 된다.
- 구 `OPS_CHECK_BACKUP_AT`은 더 이상 쓰이지 않는다. **값이 남아 있으면
  경고한다** — 조용히 무시하면 운영자는 그 시각에 돈다고 믿는다.

### 2. 백업 전용 버킷 (CTO 결정 1701-②)

원격 복제 대상이 이미지 버킷에서 **`BACKUP_BUCKET`**(기본
`<S3_BUCKET>-backups`)으로 분리됐다.

- 같은 버킷에 두면 그 버킷이 사라질 때 **이미지와 백업이 함께** 사라져,
  원격 복제의 목적을 절반만 달성한다.
- 백업 버킷에는 **공개 읽기 정책을 걸지 않는다** — 이미지 버킷과 정책이 다르다.
  덤프가 공개되면 데이터베이스 전체가 공개되는 것과 같다.
- 화면은 분리 여부를 그대로 보여준다. 같은 버킷이면 "함께 사라집니다"라고
  적는다.

### 3. Versioning 운영 필수 · Replication 권장 (CTO 결정 1701-③)

| 상태 | 개발 | 운영 |
| --- | --- | --- |
| Versioning 꺼짐 | `warn` | **`fail`** |
| Replication 꺼짐 | `warn` | `warn` |
| 조회 불가 | `manual` | **`manual` (유지)** |

둘의 무게가 다른 이유: 버전 관리가 없으면 **실수 한 번으로 되돌릴 수 없게**
되지만, 복제가 없는 것은 리전 전체가 사라지는 훨씬 드문 사건에 대한 대비가
없다는 뜻이다.

조회 불가가 운영에서도 `manual`인 것은 결정에 명시된 대로다 — **확인하지 못한
것을 "꺼짐"으로 단정하면 그것도 거짓**이기 때문이다.

### 4. 복구 리허설 (CTO 결정 1701-⑤)

**분기 1회 실제 복구가 운영 표준이 됐다.**

TASK-1601부터 체크리스트의 "복구 절차 숙지"는 `manual`로 남아 있었다. 정직한
표시였지만 `manual`은 영원히 `manual`이라 **아무도 하지 않아도 아무 일도
일어나지 않는다.** 리허설을 기록하면 그 항목이 비로소 판정 대상이 된다.

| 상태 | 조건 | 경보 |
| --- | --- | --- |
| `manual` | 기록 없음 | **없음** — 아직 시작하지 않은 것이지 규칙을 어긴 것이 아니다 |
| `pass` | 기한 안 성공 | 없음 |
| `warn` | 기한 초과, 유예(14일) 안 | warning |
| `fail` | 유예 초과 | warning |
| `fail` | **마지막 리허설이 실패** | **critical** |

- **실패한 리허설도 기록한다** — 오히려 그쪽이 값지다. 절차가 깨졌다는 사실을
  사고 전에 알아낸 것이기 때문이다. 성공만 남기면 기록은 장식이 된다.
- **수행자는 필수**다 — 누가 확인했는지 남지 않으면 기록이 아니다.
- 리허설이 밀린 것은 **복구 가능 판정을 막지 않는다** — 지금 복구가 불가능한
  것과 다르다.
- 리허설 경보는 예약 점검 경보와 **종류를 나눠 동기화**한다. 섞으면 한쪽이
  다른 쪽을 해소해 버린다.

### 5. 원격 사본 자동 검증 (CTO 결정 1701-④)

**Sprint 18 이후로 미뤘다.** 지금은 체크섬을 기록해 사람이 대조할 수 있게만
해 둔다.
...
| 변수 | 뜻 | 기본값 |
| --- | --- | --- |
| `OPS_CHECK_BACKUP_INTERVAL` | 백업 간격 | `1h` |
| `BACKUP_BUCKET` | 백업 전용 버킷 | `<S3_BUCKET>-backups` |
| `OPS_DRILL_INTERVAL_DAYS` | 리허설 주기 | 90 |
| `OPS_DRILL_GRACE_DAYS` | 기한 초과 후 경보 유예 | 14 |
| `ALERT_COOLDOWN_DRILL_MS` | 리허설 경보 재알림 간격 | 전체 기본값 |
...
`GET /ops/readiness`의 `enterprise`에 **`drill`** 과 **`backupBucket`** 이
추가됐고, 경보 종류에 **`recovery-drill`** 이 추가됐다.

---

## Enterprise Recovery Assurance (TASK-1901, Sprint 19)

CTO 결정 1801-①~⑤ 반영. 주제는 **보증(assurance)** — 지금까지 관측만 하던
값에 기준을 붙이고, 판정이 미치지 않던 곳(백업 버킷)까지 같은 규칙을 넓히고,
달력에만 묶여 있던 리허설을 **변경에 반응하게** 한다.

### 1. 백업 소요 시간 기준 (CTO 결정 1801-①)

`backup_runs.durationMs`는 TASK-1601부터 기록되고 있었다. 여기서 하는 일은
**그 값에 기준을 붙이는 것**뿐이다 — 관측만 하고 판정하지 않으면 아무도 보지
않는다.
...
- **연속 3회를 요구하는 이유**: 한 번 느린 것은 흔하다(디스크 경합, 다른
  작업과 겹침). 그때마다 경보하면 사람이 경보를 무시하게 된다.
- **30초는 한 번만 넘어도 심각**: 그 정도면 일시적 경합으로 설명되지 않는다.
- **주의(2~10초)는 경보하지 않는다** — 화면에는 드러나되 사람을 부르지 않는다.
  부를 일이 아닌데 부르면 정작 불러야 할 때 오지 않는다.
- **실패한 백업은 성능 판정에서 제외한다** — 실패는 백업 항목이 이미 잡아내고,
  실패까지 걸린 시간을 성능으로 세면 판정이 흐려진다.
- **자동으로 간격을 바꾸지 않는다.** 시스템이 스스로 백업을 드물게 만들면
  손실 구간이 조용히 늘어난다 — 그 판단은 사람이 해야 한다.

### 2. 백업 버킷 보호 (CTO 결정 1801-③)

이미지 버킷과 **같은 규칙**으로 판정한다(운영에서 Versioning 필수, Replication
권장, 조회 불가는 `manual`). 판정 문구에 **어느 버킷 이야기인지**를 넣는다 —
"버전 관리가 꺼져 있습니다"만 두 번 나오면 어느 쪽을 고쳐야 할지 알 수 없다.

라벨 뒤에 조사를 붙이지 않는다("백업 버킷**가**"처럼 어긋난다).

### 3. 운영 저장소 표준 (CTO 결정 1801-④)

**Amazon S3가 운영 표준이고, MinIO·s3rver는 개발 전용이다.** 운영에서
`S3_ENDPOINT`가 `*.amazonaws.com`이 아니면 경고한다 — 개발 전용 저장소는 버전
관리·복제 조회를 지원하지 않아, 그대로 운영에 쓰면 보호 상태를 영영
`직접 확인`으로만 보게 된다.

### 4. 변경 후 즉시 추가 리허설 (CTO 결정 1801-⑤)

**달력이 아니라 변경이 리허설을 부른다.** 90일 주기는 "아무것도 바뀌지
않았을 때"의 간격이다.
...
- 등록되면 **기한이 남아 있어도 실패**다 — 마지막 리허설이 검증한 것은 변경
  이전의 시스템이다.
- **성공한 리허설만 해소한다** — 실패한 리허설은 "확인했다"가 아니라
  "안 되더라"는 뜻이다.
- 판정과 경보의 우선순위를 **같게** 둔다: 마지막 리허설 실패 → 변경 사건 →
  기한 초과. 순서가 다르면 제목과 본문이 어긋난다(`lastFailed` 플래그로 구분).

### 5. 배포 게이트에는 넣지 않는다 (CTO 결정 1801-②)

리허설은 운영 표준이지만 **배포를 막지 않는다** — 밀렸다는 이유로 긴급 배포가
막히면 안 된다. 배포 체크리스트에 리허설·백업 성능 항목이 없음을 회귀 테스트로
고정했다.
...
`GET /ops/readiness`의 `enterprise`에 **`performance`** 와
**`backupBucket.protection`** 이, `drill`에 **`pendingTriggers`·`requirements`**
가 추가됐다. 경보 종류에 **`backup-performance`** 가 추가됐다.
