# 운영 전환·활성화 (TASK-3401 · 3501 · 3601, Sprint 34~36)
...
## 1.15 운영 활성화 — 세 조건 (CTO 정책 3601-①)

전환 판정 하나만으로는 **무엇이 남았는지** 말하지 못합니다. 키가 없는 것과
길이 막힌 것과 판정이 빨간 것은 사람이 할 일이 전혀 다릅니다. 그래서 완료
조건을 셋으로 못박았습니다 (`GET /ops/activation`).

| 조건 | 무엇을 보는가 | 충족이 아닌 경우 |
| --- | --- | --- |
| `credentials` | 골라 놓은 Provider·저장소의 자격 증명이 **형식상 있는가** | mock · 키 없음 · 플레이스홀더 · `minioadmin` |
| `network` | 그 공식 주소에 **닿는가** | 막힘 · **403(가릴 수 없음)** · **점검하지 않음** |
| `cutover` | `/ops/cutover`가 **전부 verified인가** | 하나라도 아니면 |

**셋 중 하나라도 아니면 완료가 아닙니다.** 둘이 충족된 상태는 "거의 다"가
아니라 여전히 **전환되지 않은 상태**입니다 — 문구가 그 사실을 그대로 말합니다.

두 가지를 덧붙입니다.

- **자격 증명은 형식만 봅니다.** 키가 유효한지는 실제 호출만이 알고, 그것은
  `cutover` 조건이 봅니다. 여기서 잡는 것은 "아예 없다 · 플레이스홀더가
  그대로다 · 형식이 깨졌다"입니다.
- **점검하지 않은 네트워크는 충족이 아닙니다.** 빈 목록은 "닿는다"가 아니라
  "모른다"이고, 모르는 것을 통과로 세지 않습니다.
...
| 종료 코드 | 뜻 |
| --- | --- |
| 0 | **세 조건이 모두 충족됐다** (또는 이 환경이 전환 대상이 아니다) |
| 1 | 아직 끝나지 않았다 (남은 조건·항목과 다음 할 일을 출력한다) |
| **2** | **판정 불가** — 확인하지 못한 것은 통과가 아닙니다 |

출력은 두 겹입니다: 항목별 전환 판정(LLM·Vision·S3·CI)과 그 위의 **활성화
세 조건**(CTO 정책 3601-①).
...
`pnpm check:ci-gates` → `pnpm typecheck` → `pnpm lint` → `pnpm test:unit` ·
`pnpm test:e2e`.

**Core/API와 Web e2e는 병렬 Job입니다** (TASK-3601, 정책 3601-④). 한 Job이
전부를 돌면 e2e가 늘 때마다 모든 게이트의 답이 늦어지고, **게이트가 느려지면
사람은 게이트를 안 기다립니다.** 두 Job은 서로를 기다리지 않고, 어느 쪽이
깨졌는지도 색으로 바로 갈립니다.
