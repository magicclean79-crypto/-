# Disaster Recovery (TASK-1601 ~ 2301, Sprint 16~23)
...
> 운영 배포 절차는 [deployment-checklist.md](deployment-checklist.md)에 있습니다
> (운영 표준, CTO 결정 2201-④).
...
- 관측 창: `BACKUP_CHAIN_WINDOW_HOURS` (기본 24시간, CTO 결정 2001-①).
  **최소는 백업 간격의 4배**이며, 그보다 짧게 두면 자동으로 올립니다.
  창이 간격의 4배도 안 되면 창 안에 백업이 서너 개뿐이라 한 번만 걸러도
  판정이 뒤집힙니다 — 표본이 없는 판정은 판정이 아니라 잡음입니다.

  **상향은 Readiness 항목 `사슬 관측 창 설정`에 Warning으로 드러납니다**
  (CTO 결정 2101-②). 해석할 수 없는 값을 적은 경우도 마찬가지입니다.
  **기동은 막지 않습니다** — 판정 설정 하나 때문에 서비스가 뜨지 않으면 안
  됩니다. 화면 한 줄로만 두면 아무도 고치지 않으므로, 그 사이에서
  **드러내되 세우지는 않는** 자리를 골랐습니다
...
- **운영에서는 주 1회 자동으로 돕니다** (CTO 결정 2001-②). 예약 점검
  ...
  **운영이 아니면 기본이 꺼짐**입니다 — 개발이 전송 비용을 낼 이유가 없고,
  ...
  환경변수를 명시하면 됩니다.
- **자동은 마지막 1건, 수동은 최근 3건까지** (CTO 결정 2101-①).
  주기적으로 도는 것은 비용이 반복되므로 최소로 두고, 사람이 필요할 때 한 번
  더 보는 쪽에만 폭을 줍니다. **상한을 두는 이유**는 "전부"를 허용하면 이력이
  쌓인 뒤 한 번의 클릭이 예상치 못한 전송 비용이 되기 때문입니다.

  ```bash
  # 최근 3건 (상한). count를 넘겨 적어도 3건까지만 봅니다
  curl -X POST "http://localhost:4000/ops/backup/verify-remote?count=3" -b cookies.txt
  ```

  **하나라도 실패하면 실패**입니다 — 3건 중 2건이 온전해도, 잃은 1건은 그
  시점으로 되돌아갈 수 없다는 뜻입니다. 건마다 결과가 남으므로 **어느 백업이
  온전한지**를 보고 복구 시점을 고를 수 있습니다.
...
#### 실패했을 때 — 자동 복구는 하지 않습니다 (CTO 결정 2201-②)

**경보만 냅니다. 다시 올리지도, 스스로 고치지도 않습니다.**

자동으로 다시 올리면 두 가지가 위험합니다.

- 지금 로컬에 있는 덤프가 온전한지 확인하지 않은 채 원격을 덮어써,
  **틀린 사본을 더 확실하게** 만들 수 있습니다.
- 원격이 사라진 이유(수명 주기 정책·권한·사람의 삭제)를 모르는 채로 다시
  올리면 같은 일이 반복됩니다.

복구 순서:

1. **로컬 덤프를 먼저 확인합니다.** `/admin/operations`의 `덤프 무결성`이
   통과인지 보고, 아니면 `POST /ops/backup/run`으로 새 백업을 받으세요.
   원격을 고치기 전에 **되돌아갈 지점이 하나는 온전해야** 합니다.
2. **왜 사라졌는지 봅니다.**
   - `missing` — 저장소 수명 주기 정책, 사람의 삭제, 버킷 교체.
     콘솔의 객체 이력(Versioning이 켜져 있으면 삭제 마커)을 확인하세요.
   - `mismatch` — 업로드 중단 또는 나중에 덮어쓰기. 같은 키로 두 번 올린
     배포가 있었는지 보세요.
3. **원인을 고친 뒤에** 백업을 한 번 실행해 새 사본을 올립니다
   (`POST /ops/backup/run`).
4. **대조로 확인합니다** — `POST /ops/backup/verify-remote?count=3`.
   경보는 대조가 통과하면 자동으로 해소됩니다.

2번을 건너뛰고 3번만 하면 다음 주에 같은 경보가 옵니다.
...
#### 이 판정이 유일한 원천입니다 (CTO 결정 2301-①)

`/ops/readiness`의 `recoverable`이 **복구 판정의 단일 원천(Single Source of
Truth)**입니다. 배포 체크리스트(`/health/ready`)의 `재해 복구 판정` 항목은
같은 계층을 호출해 **결론만 옮깁니다** — 판정을 다시 하지 않습니다.

| 보는 곳 | 무엇을 보여주는가 |
| --- | --- |
| `/ops/readiness` | 판정의 **근거** — 항목별 통과·실패·직접 확인 |
| `/health/ready`의 `readiness` 항목 | 판정의 **결론** — 통과 / 실패(배포 차단) / 직접 확인 |

TASK-2401 이전에는 배포 체크리스트가 순환 의존을 피하려고 **백업·복원 이력
개수**로 대신 판정했습니다. 그래서 백업과 복원 기록이 있으면 "복구 가능"으로
읽혔고, 실제 복구 판정이 실패인데도 두 화면이 다른 말을 할 수 있었습니다.
지금은 두 화면이 어긋날 수 없습니다.

판정 자체를 확인하지 못하면 배포 체크리스트에서 `직접 확인`으로 남습니다 —
**확인하지 못한 것을 통과로 세지 않습니다.**
