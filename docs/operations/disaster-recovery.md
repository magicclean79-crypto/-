# Disaster Recovery (TASK-1601 · 1701 · 1801 · 1901, Sprint 16~19)
...
**운영 저장소 표준은 Amazon S3입니다.** MinIO·s3rver는 **개발 전용**이며,
버전 관리·복제 조회를 지원하지 않아 보호 상태를 자동으로 확인할 수 없습니다.
운영에서 `S3_ENDPOINT`가 `*.amazonaws.com`이 아니면 경고합니다.
...
curl -X POST http://localhost:4000/ops/drills/require -b cookies.txt \
  ...
  -d '{"trigger":"db-major-change","description":"주문 테이블 분할","registeredBy":"홍길동"}'
```
