    ...
    build(context) {
      const systemLines = [
        ...
        "- 필드: material(재질, 문자열 또는 null) · color(색상, 문자열 또는 null) · " +
          ...
          "confidence(0.0~1.0 숫자) · photoTypes(문자열 배열) · photoCaptions(문자열 " +
          "또는 null 배열)",
        "- photoTypes는 첨부된 이미지 순서와 정확히 1:1 대응하는 배열이다(이미지 " +
          ...
          "판단해서 순서대로 채운다.",
        "- photoCaptions도 첨부된 이미지 순서와 정확히 1:1 대응하는 배열이다. 상세" +
          "페이지에서 **바로 그 사진 옆에** 붙일 짧은 한 문장(20~60자)을 이미지마다 " +
          "따로 쓴다 — 이 사진에서 실제로 보이는 것(부위·동작·구성·사용 장면)만 " +
          "묘사한다. 여러 사진에 같은 문장을 복사해 쓰지 않는다 — 사진마다 실제로 " +
          "다른 부분이 보이면 그 차이를 쓴다. photoTypes가 INFO인 이미지는 항상 " +
          'null을 쓴다. DESIGN이어도 사진만으로 확신 있게 설명할 것이 없으면(예: ' +
          "너무 흐릿하거나 이미 다른 사진과 완전히 같은 장면) 지어내지 말고 null로 둔다.",
