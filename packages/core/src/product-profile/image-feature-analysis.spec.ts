...
describe("parseImageFeatureAnalysisResponse", () => {
  ...
  it("JSON 객체 응답을 ImageFeatureAnalysis로 파싱한다", () => {
    expect(parseImageFeatureAnalysisResponse(JSON.stringify(valid))).toEqual(
      valid,
    );
  });
  ...
  it("코드 펜스·해설이 섞인 응답에서도 JSON을 추출한다", () => {
    ...
    expect(parseImageFeatureAnalysisResponse(text).material).toBe("PVC");
  });

  it("JSON을 찾을 수 없으면 ImageFeatureParseError", () => {
    expect(() => parseImageFeatureAnalysisResponse("그냥 텍스트")).toThrow(
      ImageFeatureParseError,
    );
    expect(() => parseImageFeatureAnalysisResponse("[1, 2]")).toThrow(
      ImageFeatureParseError,
    );
  });

  it("모든 필드가 없어도 안전한 기본값(null·빈 배열·0.5)으로 보정한다", () => {
    expect(parseImageFeatureAnalysisResponse("{}")).toEqual({
      material: null,
      color: null,
      structure: null,
      usage: null,
  ...
  it("confidence는 0~1로 클램프한다", () => {
    expect(
      parseImageFeatureAnalysisResponse(JSON.stringify({ confidence: 7 }))
        .confidence,
    ).toBe(1);
    expect(
      parseImageFeatureAnalysisResponse(JSON.stringify({ confidence: -3 }))
        .confidence,
    ).toBe(0);
  });
