...
/**
 * Memory Engine — 기억의 기록/회상을 담당하는 도메인 서비스. (TASK-0307)
 *
...
export class MemoryEngine {
  ...
  async remember(input: CreateMemoryInput): Promise<Memory> {
    const errors = validateCreateMemory(input);
    if (errors.length > 0) {
  ...
  /** 프로젝트의 기억을 회상한다 (최신순) */
  async recall(projectId: string): Promise<Memory[]> {
  ...
  async revise(id: string, input: UpdateMemoryInput): Promise<Memory> {
    const errors = validateUpdateMemory(input);
    if (errors.length > 0) {
      throw new MemoryValidationError(errors);
    }
