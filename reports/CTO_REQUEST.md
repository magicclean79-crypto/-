...
### 54. TASK-2101 "Enterprise Operational Automation" 세부 해석 확인
- 현황: CTO 결정 2001-①~⑤가 이번 TASK의 범위였습니다. 주제는 **사람이
  기억하지 않아도 도는 것**입니다. TASK-2001은 판정을 만들었고, 이번에는 그
  판정이 **누가 누르지 않아도** 유지되게 했습니다. 규칙은 있는데 아무도
  실행하지 않으면 규칙이 없는 것과 같습니다 — 원격 대조 버튼이 그 상태였습니다:
  - **관측 창 설정**(결정 2001-① — `BACKUP_CHAIN_WINDOW_HOURS`, 기본 24시간.
    **최소는 백업 간격 × 4**이고 그보다 짧으면 자동으로 올립니다. 창이 간격의
    4배도 안 되면 창 안에 백업이 서너 개뿐이라 한 번만 걸러도 판정이 뒤집힙니다.
    **설정을 거절하고 판정을 멈추는 대신 올려서 쓰고, 올렸다고 화면에 적습니다**
    — 조용히 다른 값으로 도는 것이 가장 나쁩니다. 기본값 24시간조차 최소에 못
    미치는 경우(일 1회 백업 → 최소 4일)도 같이 올립니다)
  - **원격 대조 주 1회 자동**(결정 2001-② — 예약 점검 `remote-verify` 신설.
    **마지막 백업 1건만** 내려받습니다. `PRODUCTION_ONLY_JOBS`로 **운영에서만**
    기본 켜짐이며, 환경변수로 명시하면 스테이징에서도 켤 수 있습니다.
    **결과를 `backup_runs`에 남깁니다** — 남기지 않으면 자동 대조를 돌려도
    화면은 계속 "확인하지 않았습니다"라고 말합니다. 화면·경보는 **기록을
    읽습니다**(조회마다 내려받으면 화면을 여는 것만으로 돈이 나갑니다).
    **주기의 2배를 넘긴 성공은 통과로 두지 않고**, 반대로 **실패는 낡아도
    실패입니다** — 시간이 지난다고 사본이 돌아오지 않습니다)
  - **`_major_` 규칙 + CI 교차 검증**(결정 2001-③ — 매니페스트를 유지하면서
    파일명 규칙을 더하고 **합집합**을 씁니다. 매니페스트는 **잊기 쉽고**
    파일명은 **되돌리기 쉬워** 서로 다른 실수를 막습니다.
    `pnpm check:major-migrations`가 한쪽에만 있는 지정을 exit 1로,
    읽지 못하면 **exit 2**로 막습니다 — 판정 불가를 통과로 처리하지 않습니다.
    `.github/workflows/ci.yml` 신설)
  - **재기동 후 확인 절차**(결정 2001-④ — 기동 시 1회 정책을 **유지**하는 대신
    결과를 남깁니다. `registered: null`은 **확인하지 못한 것**이고 "등록할 것이
    없었다"와 구분됩니다. readiness `enterprise.drill.autoRegistration`과
    화면 칸으로 노출하고 Runbook에 3단계 절차를 넣었습니다 — 로그를 뒤지게
    만들면 아무도 확인하지 않습니다)
  - **S3 전환 계획**(결정 2001-⑤ — `docs/operations/s3-migration.md`.
    **핵심은 저장소를 바꾸는 것이 아니라 `s3:GetBucketVersioning`·
    `s3:GetReplicationConfiguration` 조회 권한을 빠뜨리지 않는 것**입니다.
    권한이 없으면 전환해도 보호 상태가 `직접 확인`으로 남아 바꾼 의미의 절반이
    사라집니다. 완료 판정을 네 항목으로 못 박았습니다)
  - 라이브(**실 PostgreSQL + 실 Redis + 실 S3**): 창 2시간이 4시간으로 상향되고
    사유가 값에 포함됨 · 기동 로그 `remote-verify — 7일` · 실 S3에서 내려받아
    SHA-256 일치 후 DB 기록 · 조회가 재다운로드 없이 기록을 읽음 · 15일 전
    성공이 `직접 확인`으로 되돌아감 · `missing` 기록에 watchdog이 **CRITICAL**
    발생 · CI 교차 검증 3경로 실측(0/1/1) · 재기동 후 `registered: true`
  - **자체 발견 결함 2건 — 둘 다 라이브에서만 드러났습니다**: ⓐ **주 1회가
    `604800초`로 찍혀 아무도 읽을 수 없었습니다** — 주기를 확인하려고 보는
    로그가 확인을 막았습니다 ⓑ **"2.0시간로 두면"** — 단위가 초·분·시간·일로
    바뀌면 받침이 달라집니다. TASK-1901의 "백업 버킷가"와 **같은 결함 계열이며
    세 번째 재발**이라, 이번에는 단위 4종을 모두 도는 회귀 테스트로 막았습니다
- 하지 않은 것: 대조 대상 확대(마지막 1건만), 창 상향의 경보화(화면에만
  적습니다), GitHub Actions 실제 실행 확인(스크립트 3경로만 실측),
  Amazon S3 실제 전환(계획만 세웠습니다)
- 질문: ① **대조 대상 범위** — 마지막 백업 1건만 봅니다. 그 앞의 원격 사본이
  사라져도 알 수 없습니다. 비용과 맞바꾼 선택인데, 표본을 늘릴지(예: 주 1회에
  최근 3건) 지금대로 둘지 ② **창 상향의 강도** — 지금은 조용히 올리고 화면에만
  적습니다. 설정과 실제가 다른 상태가 계속될 수 있는데, 경보까지 낼지 아니면
  기동을 막을지(Fail Fast) ③ **CI 게이트 위치** — 교차 검증을 build 뒤 lint
  앞에 두었습니다. 이 저장소에서 GitHub Actions가 실제로 도는지 확인하지
  못했으니, 워크플로 파일이 맞는 위치·이름인지 알려 주십시오 ④ **S3 전환
  주체·일정** — 계획은 섰습니다. 버킷 생성과 IAM 발급은 코드가 할 수 없는
  일이라, 누가 언제 하는지와 자격 증명 전달 경로를 정해 주십시오
  ⑤ **Sprint 21 진행 방향** 및 다음 TASK 지정 요청.
