...
### 49. TASK-1601 "Production Verification & Operational Readiness" 세부 해석 확인
- 현황: 지난 Sprint까지가 "잘 돌고 있는가"였다면, 이번 주기의 질문은
  **"지금 무너지면 되살릴 수 있는가"** 였습니다. 지금까지의 운영 기능이
  **한 번도 확인되지 않은 전제** 위에 서 있었기 때문입니다 — 백업은 받은 적이
  없고, 복원은 해 본 적이 없고, 메일 경로는 살아 있는 서버로 검증된 적이
  없었습니다:
  - **Real SMTP Validation**: 실제 SMTP에 연결·인증까지 하되 **메일은 보내지
    않습니다**(점검이 수신함을 채우면 사람이 점검 메일을 무시하게 되고, 그러면
    진짜 경보도 함께 묻힙니다). 미설정은 실패가 아니라 **미구성**으로 알리고,
    결과에 `ALERT_EMAIL_TO`를 담지 않습니다(수신자 주소는 비밀입니다).
    **이번에 처음으로 살아 있는 SMTP 서버와 실제 EHLO→AUTH→QUIT를 주고받아
    검증했습니다** — TASK-1401부터 세 번의 보고서에 올렸던 부채를 갚았습니다
  - **Backup Automation**: `pg_dump --format=custom`, 매일 03:00(로컬).
    **크기를 기록합니다** — `pg_dump`는 부분 실패에도 0에 가까운 파일을 남길 수
    있고 그걸 성공으로 세면 **복구 계획이 통째로 거짓이 됩니다**(1KB 미만은
    실패). 보존 기간이 지나도 **최근 3개는 남깁니다**(정리가 마지막 백업을
    지우면 안 됩니다). 실패해도 예외를 던지지 않고 이력에 남깁니다(예약 실행이
    죽으면 다음 백업도 못 받습니다). 화면·API에는 **파일명만** 노출합니다
  - **Restore Verification**: 매일 03:30(로컬), 최신 덤프를 **별도 DB**에
    복원하고 테이블 수를 셉니다. **운영 DB에는 절대 복원하지 않습니다** —
    검증하려다 데이터를 잃는 것이 최악입니다. 대상이 없으면 **미구성**입니다
    (하지 못한 것과 실패한 것은 다릅니다). `pg_restore`는 무해한 경고에도
    비영점 종료하므로 판정은 종료 코드가 아니라 **복원 후 테이블 수**로 합니다.
    **복원해 보지 않은 백업은 백업이 아니므로** 이력이 없으면 `missing`이고
    그것은 통과가 아닙니다
  - **Disaster Recovery Checklist**: 배포 체크리스트(1202)와 질문이 다릅니다 —
    그쪽은 "지금 배포해도 되는가", 이쪽은 "지금 무너지면 되살릴 수 있는가".
    백업·복원·DB·저장소가 복구를 좌우하고 Redis·경보 채널은 아닙니다(Redis가
    죽어도 복구는 할 수 있습니다). 복구 절차 숙지·연락 체계는 자동 판정이
    불가능하므로 **`직접 확인`으로 남깁니다** — 모르는 것을 통과로 처리하지
    않습니다
  - **Provider Smoke Automation**: 예약 자리는 있지만 **기본은 꺼져 있습니다**
    (실제 과금되므로, 결정 1301-①). 구현 중 **"지금 점검"이 꺼진 과금 작업까지
    실행하는 것을 발견해 고쳤습니다**
  - **결정 1501-①~④ 반영**: ① 일 1회 점검 4종을 **운영 서버 로컬 시각**으로
    (`TZ`) ② Redis 장애 시 **LLM 호출은 계속 허용**, Scheduler만 중단, 30분
    이상 지속되면 Critical 반복 ③ **Dead Letter도 지우지 않고** 90일 후 Archive
    ④ Grace Factor 3배 유지, `OPS_SCHEDULER_GRACE_<JOB>`으로 Job별 조정
  - 라이브(**실 PostgreSQL + 실 Redis + 실 SMTP + 실 pg_dump/pg_restore**):
    초기 `복구 불가`(백업·복원 이력 없음) → **백업 4회 성공**(수동 1·예약 3) →
    **복원 검증 4회 모두 테이블 30개 확인** → `복구 가능`. **Redis를 내린 채
    LLM 호출이 HTTP 200으로 성공**하고 예약 점검만 멈추며 Critical 경보 발생,
    복구 시 RESOLVED, 재중단 시 재발생
  - **스스로 발견해 고친 결함 4건**: ⓐ "지금 점검"이 **과금되는 스모크**를
    실행 ⓑ Prisma의 `?schema=public`을 `pg_dump`가 거부해 **백업이 100% 실패**
    (라이브에서만 드러났습니다) ⓒ 기동 로그·화면이 "04:00 UTC"라고 표시(실제로는
    로컬로 도는데 설명만 UTC라 운영자가 다른 시각을 기다립니다) ⓓ 경보 문구의
    마크다운 `**`가 Slack·메일·화면에 별표로 그대로 노출
- 하지 않은 것: 이미지 저장소 백업(S3 버킷 정책에 의존하며 체크리스트가 자동
  판정하지 않습니다), 증분 백업·PITR(WAL 아카이빙), 복원 후 행 수·무결성 검증
  (테이블 수까지만 봅니다), 실 Provider 스모크(샌드박스에서 `api.openai.com`
  차단 + 과금)
- 질문: ① **백업 저장 위치** — `BACKUP_DIR` 기본값이 컨테이너 안이라 볼륨을
  붙이지 않으면 백업이 컨테이너와 함께 사라집니다. 운영에서 미설정이면
  경고하지만 **기동을 막지는 않습니다**. 이 정도로 둘지, 운영 필수(누락 시 기동
  차단)로 올릴지 ② **복원 검증 대상 DB** — `BACKUP_RESTORE_DB_URL`은 운영자가
  별도로 만들어야 하고, 없으면 "복원해 본 적 없는 백업" 상태로 남습니다. 이 DB를
  운영 인프라 어디에 둘지(같은 인스턴스의 별도 DB / 별도 인스턴스) 지시 요청
  ③ **백업 방식** — 지금은 전체 덤프뿐이라 최대 손실 구간이 **마지막 백업 이후
  전부**입니다. 증분·PITR(WAL 아카이빙)이 필요한 수준인지 ④ **이미지 저장소
  백업** — DB만 복원하면 이미지 참조가 깨질 수 있는데, 지금은 저장소 제공자의
  버킷 정책에 의존하고 체크리스트가 자동 판정하지 않습니다. 이 항목을 어떻게
  다룰지 ⑤ **백업 신선도 기준** — 백업 2일·복원 검증 8일을 기준으로 두었습니다
  (하루 1회 백업 기준). 이 값이 적절한지 ⑥ **Sprint 16 진행 방향** 및 다음 TASK
  지정 요청.
