...
### 55. TASK-2201 "Enterprise Production Readiness" 세부 해석 확인
- 현황: CTO 결정 2101-①~④가 이번 TASK의 범위였습니다. 주제는 **운영에 넘길 수
  있는 상태**입니다. 지금까지 시스템은 필요한 것을 스스로 준비해 왔습니다 —
  버킷을 만들고, 정책을 걸고, 잘못된 설정을 조용히 고쳐서. 개발에서는
  편하지만 **운영에서는 그것이 곧 책임의 실종**입니다. 이번 TASK는 그 손을
  놓고, 대신 **무엇이 필요한지 말하고 기다리는** 쪽으로 바꿉니다:
  - **수동 대조 최대 3건**(결정 2101-① — 자동(주 1회)은 1건 그대로입니다.
    주기적으로 도는 것은 비용이 반복되므로 최소로 두고, 사람이 필요할 때 한 번
    더 보는 쪽에만 폭을 줍니다. **상한을 두는 이유**는 "전부"를 허용하면 이력이
    쌓인 뒤 한 번의 클릭이 예상치 못한 전송 비용이 되기 때문입니다.
    `count=99`는 3으로 조이고 `count=all`은 400으로 거절합니다.
    **하나라도 실패하면 실패**입니다 — 3건 중 2건이 온전해도 잃은 1건은 그
    시점으로 되돌아갈 수 없다는 뜻이라, 평균을 내면 그 사실이 사라집니다.
    **건마다 결과를 남깁니다** — 어느 백업이 온전한지가 복구 시점 선택의
    근거입니다)
  - **관측 창 상향은 Warning**(결정 2101-② — 체크리스트 항목 `chain-window`로
    승격했고 `critical: false`라 복구 가능성 판정을 바꾸지 않습니다. **기동은
    막지 않습니다.** 화면 한 줄로만 두면 아무도 고치지 않고, 기동을 막으면
    판정 설정 하나 때문에 서비스가 뜨지 않습니다 — **드러내되 세우지는 않는**
    자리를 골랐습니다. 여기서 **기존 결함**을 발견했습니다:
    `BACKUP_CHAIN_WINDOW_HOURS`에 `validate`가 걸려 있어 해석 불가 값이면
    **Fail Fast로 기동이 막혔는데**, 코드는 조용히 기본값으로 돌고 있었습니다.
    검증과 동작이 정반대였고 이 결정과도 충돌해, 경고로 낮추고 무시한 사실을
    `source: "invalid"`로 드러냈습니다)
  - **CI 게이트 위치 고정**(결정 2101-③ — Build 이후 유지. 앞으로 옮기면 검증이
    `@acos/core` 빌드 산출물을 못 찾아 **조용히 exit 2**가 되고 그것이
    "판정 불가"로 취급됩니다. `ci-workflow.spec.ts`가 워크플로 파일을 읽어
    순서를 고정합니다 — 기록되지 않는 규칙은 지켜지지 않습니다)
  - **버킷·IAM은 운영 담당자가**(결정 2101-④ — 운영에서 애플리케이션은 **버킷을
    만들지도, 정책을 걸지도 않습니다.** 이유 셋: ⓐ 오타 하나로 엉뚱한 이름의
    버킷이 조용히 생기고 그 버킷은 백업 대상도 보호 정책 대상도 아닙니다
    ⓑ 애플리케이션이 정책을 덮어쓰면 운영자가 콘솔에서 조인 권한이 다음
    재기동에 풀립니다 ⓒ 누가 무엇을 책임지는지가 흐려집니다. 버킷이 없으면
    **누가 무엇을 해야 하는지**까지 적습니다 — "찾을 수 없습니다"로 끝내면
    운영자는 앱이 만들어 주기를 기다립니다. 운영에서 `putObject` 실패 문구도
    고쳤습니다 — "MinIO가 실행 중인지 확인하세요"는 운영에서 거짓 안내입니다)
  - 라이브(**실 PostgreSQL + 실 Redis + 실 S3**): 해석 불가 창 설정으로도
    **기동됨**(권고 로그만) · `chain-window`가 `warn`이고 복구 가능성을 낮추지
    않음 · 실 백업 3건을 내려받아 **서로 다른 SHA-256 3개** 대조 후 건마다 DB
    기록 · `count=99` → 3건 · `count=all` → **400** · 운영에서 없는 버킷을
    가리키자 **s3rver에 버킷이 생기지 않았고** 안내가 로그·체크리스트에 그대로
    노출 · 개발 모드에서는 같은 조건에서 버킷이 생성됨
  - **자체 발견 결함 2건**: ⓐ 위의 `validate` Fail Fast 모순 ⓑ **선언한 DTO가
    실제 응답과 달랐습니다** — `RemoteVerifyResultDto.entries`를
    `{fileName, verdict, detail}`로 적었는데 실제는 `{fileName, result:{…}}`
    였습니다. 타입 수준의 "상태와 설명 모순"이라 DTO를 실제에 맞추고 컨트롤러
    반환 타입으로 묶었습니다
- 하지 않은 것: 마이그레이션 실행 주체의 경계 명시(저장소만 다뤘습니다),
  대조 실패 시 자동 재업로드, `NODE_ENV` 외의 환경 구분, Amazon S3 실제 전환
- 질문: ① **프로비저닝 경계의 범위** — 저장소에만 적용했습니다. DB 스키마
  (`prisma migrate deploy`)도 애플리케이션 밖에서 돌지만 그 경계는 코드로
  강제하지 않았습니다. 같은 규칙을 적용할지, 문서로만 남길지 ② **대조 실패 시
  조치** — 지금은 경보만 냅니다. 원격 사본이 깨졌을 때 자동으로 다시 올릴지,
  사람이 판단할지 ③ **스테이징 구분** — `NODE_ENV=production`으로 띄운
  스테이징에서도 버킷을 만들지 않습니다. 의도한 동작이지만 구축이 번거로워지는데,
  별도 플래그(`STORAGE_PROVISIONING=managed`)를 둘지 ④ **운영 배포 시 주의** —
  기존 운영이 앱의 자동 생성에 의존하고 있었다면 이번 변경 후 **버킷이 이미
  있어야 합니다.** 전환 전에 확인이 필요한지 ⑤ **Sprint 22 진행 방향** 및
  다음 TASK 지정 요청.
