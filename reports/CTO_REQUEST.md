...
### 46. TASK-1302 "Production Automation & Alerting" 세부 해석 확인
- 현황: 지시 8항목을 다음과 같이 구현했습니다. 설계의 축은 **"사람이 보고 있지
  않을 때"** 입니다 — TASK-1301은 좋은 점검을 만들었지만 **사람이 화면을 열어야**
  결과를 볼 수 있었고, 예산이 밤사이 터지거나 키가 폐기되면 다음 날 아침에야
  알게 됩니다:
  - **Scheduled Cost Verification / Provider Validation / Health Check**:
    각각 15분·15분·1시간 간격(`OPS_CHECK_*_INTERVAL`, `15m`·`1h`·`30s`·ms 모두
    해석, `off`로 중단). 해석할 수 없는 값은 기본값으로 돕니다 — 잘못 적은 값
    때문에 점검이 멈추거나 폭주하는 것보다 안전합니다. **점검마다 책임지는
    경보 종류를 나눴습니다** — 해소 판정은 "이번에 감지되지 않았다"로 하므로,
    비용 점검이 돌 때 Provider 경보까지 해소하면 **실제로는 죽어 있는 Provider가
    조용히 사라집니다**
  - **예약 점검은 Live Check를 하지 않습니다** (결정 1301-①) — 예약이 실 키를
    자동 호출하면 과금이 사람 모르게 발생합니다. Health Check 점검은 이미 쌓인
    Execution을 읽을 뿐 새 호출을 만들지 않습니다
  - **Budget / Provider Failure / Unpriced Model / Configuration Alert** 4종.
    경보 체계에서 가장 중요한 판단은 **"언제 알리지 않을 것인가"** 라고 보고
    다음을 넣었습니다: 같은 문제는 쿨다운(기본 30분) 안에서 **재알림 억제**
    (같은 내용이 반복되면 사람이 경보를 무시하기 시작하고 그 순간 경보 체계는
    없는 것과 같아집니다) / **심각도 상승은 쿨다운 무관 알림** / **해소도 알림**
    (났다는 사실만 알리면 지금 문제가 있는지 알 수 없습니다) / **판정 불가
    (`unknown`)는 경보하지 않음** / 예산이 무제한이면 예산 경보 없음
  - **전달 채널**: 로그(항상) + 웹훅(`ALERT_WEBHOOK_URL`). **웹훅 실패가 점검을
    실패시키지 않습니다** — 알림 채널이 죽었다고 감지까지 멈추면 상황이 더
    나빠집니다. 웹훅 주소는 API로 노출하지 않습니다(설정 여부만)
  - **CI/CD Deployment Gate** (결정 1202-④): `/health/ready` 자동 판정 —
    exit `0` 가능 / `1` 불가 / **`2` 판정 불가**. 판정할 수 없을 때 0을 돌려주면
    게이트가 있으나 마나 하므로, **확인하지 못한 것은 통과로 처리하지 않습니다**
  - **결정 1301-③ 이행**: 진단 호출을 **feature를 추가하지 않고**
    `executions.diagnostic` 메타데이터 한 칸으로 구분해(마이그레이션 26)
    Production Monitoring에서 제외했습니다. 다만 **숨기지는 않고**
    `diagnosticCalls`로 제외된 수를 함께 보여 줍니다
  - **결정 1301-② 이행**: `LLM_MONITOR_HEALTHY_RATE`/`DEGRADED_RATE`/
    `MIN_SAMPLES`/`P95_WARN_MS`. 미설정 시 확정 기본값(0.95/0.5/5/20000)이
    그대로 쓰이고, 해석 불가·**순서가 뒤집힌 기준**(degraded>healthy)은 둘 다
    기본값으로 되돌립니다 — 이상한 기준으로 조용히 판정하지 않기 위해서입니다
  - 라이브: 미인증 **401 2/2** · **예약 점검이 개입 없이 스스로 돌아**
    미산정 경보를 자동 감지(`trigger=schedule`) · 이후 수동 실행 2회는
    `alertsRaised=0`으로 **억제**되고 `occurrences`만 누적 · 예산 0.01로 낮춰
    `raise [critical]` → 99로 복구해 `resolve` · 사용자 호출 8건/진단 3건에서
    모니터링이 **8건만 집계**하고 `diagnosticCalls=3` 표시 · 게이트 **exit
    0/1/2 3경로** 모두 확인
- **구현 중 발견해 고친 표시 결함 2건**: ① 주의 경보만 있을 때 배지가
  **"이상 없음"** 인데 바로 아래에 경보를 나열해 **배지와 목록이 정면으로
  모순**되었습니다(1202에서 고친 것과 같은 부류) — `주의 N건`으로 분리하고 회귀
  테스트를 남겼습니다. ② 비용 검증 메시지의 마크다운 강조(`**…**`)가 화면에
  평문으로 그대로 찍혀 core에서 제거하고 테스트로 고정했습니다.
- 하지 않은 것: 다중 인스턴스 예약 조율(분산 락 — 현재는 인스턴스마다 중복
  실행되며 경보 중복은 `key` 유니크로 막히지만 알림은 경합에서 중복될 수
  있습니다), 웹훅 재시도·다중 채널, 게이트의 실제 CI 파이프라인 연결(인프라
  영역), 과거 진단 호출의 소급 구분
- 질문: ① **경보 쿨다운·채널 정책** — 지금은 모든 종류가 동일하게 30분이고
  채널은 단일 웹훅입니다. 종류별로 다르게 할지(예산 초과는 더 자주, 미산정
  모델은 더 드물게), 채널을 늘릴지(메일·Slack) ② **다중 인스턴스** — 예약
  점검이 인스턴스마다 돕니다. 분산 락/리더 선출을 도입할지, 현재처럼 두고
  중복 알림을 감수할지(경보 자체는 `key`로 중복되지 않습니다)
  ③ **게이트 기본 정책** — 현재 `GATE_STRICT`·`GATE_ALERTS`는 **꺼짐**이
  기본이라 직접 확인 항목이나 활성 critical 경보가 있어도 배포가 통과합니다.
  운영 배포에서는 켜는 것을 기본으로 할지 ④ **경보 보존** — 해소된 경보를
  지우지 않고 계속 쌓습니다. 보존 기간을 둘지 ⑤ **Sprint 13 종료 여부** —
  실 Provider 통합(1301)과 운영 자동화(1302)로 Sprint 목표가 채워졌는지
  ⑥ 다음 TASK 지정 요청.
