...
### 52. TASK-1901 "Enterprise Recovery Assurance" 세부 해석 확인
- 현황: CTO 결정 1801-①~⑤가 이번 TASK의 범위였습니다. 주제는 **보증**입니다 —
  관측만 하던 값에 기준을 붙이고, 판정이 미치지 않던 곳까지 같은 규칙을 넓히고,
  달력에만 묶여 있던 리허설을 **변경에 반응하게** 만들었습니다:
  - **백업 소요 시간 기준**(결정 1801-① — 2초 미만 정상 / 2~10초 주의 /
    10초 초과 **연속 3회** 경보 / 30초 초과 심각. `backup_runs.durationMs`는
    이미 기록되고 있었고, 이번에 한 일은 **그 값에 기준을 붙이는 것**뿐입니다.
    **주의(2~10초)는 경보하지 않습니다** — 화면에만 드러냅니다. 부를 일이
    아닌데 부르면 정작 불러야 할 때 오지 않습니다. **실패한 백업은 성능
    판정에서 제외**합니다(실패는 백업 항목이 이미 잡아냅니다). **자동으로
    간격을 바꾸지 않습니다** — 시스템이 스스로 백업을 드물게 만들면 손실
    구간이 조용히 늘어납니다)
  - **백업 버킷 보호 판정**(결정 1801-③ — 이미지 버킷과 같은 규칙으로 판정하되
    문구에 **어느 버킷 이야기인지**를 넣었습니다. "버전 관리가 꺼져 있습니다"만
    두 번 나오면 어느 쪽을 고쳐야 할지 알 수 없습니다)
  - **운영 저장소 표준 = Amazon S3**(결정 1801-④ — 운영에서 `S3_ENDPOINT`가
    `*.amazonaws.com`이 아니면 경고합니다. 개발 전용 저장소를 그대로 운영에
    쓰면 보호 상태를 영영 `직접 확인`으로만 보게 됩니다)
  - **변경 후 즉시 추가 리허설**(결정 1801-⑤ — **달력이 아니라 변경이 리허설을
    부릅니다.** `dr-change`·`db-major-change`·`pitr-adoption`이 등록되면
    **기한이 남아 있어도 실패**입니다. **성공한 리허설만 해소**합니다 —
    실패한 리허설은 "확인했다"가 아니라 "안 되더라"는 뜻입니다)
  - **배포 게이트 미포함**(결정 1801-② — 현행 유지를 확인하고, 배포 체크리스트에
    리허설·백업 성능 항목이 없음을 **회귀 테스트로 고정**했습니다. 확인만 하고
    지나가면 다음에 누군가 무심코 넣어도 아무도 모릅니다)
  - 라이브(**실 PostgreSQL + 실 Redis + 실 S3 + 실 SMTP**): 실 이력 기준 성능
    `normal`(225ms) · 35초 주입 시 **CRITICAL 발생** · **변경 사건 등록 시 다음
    예정일이 89일 남았는데도 리허설이 `fail`로 전환**되고 warning 발생 ·
    **실패한 리허설은 요구를 해소하지 못하고** 성공한 리허설이 해소 · 백업
    버킷이 이미지 버킷과 별도로 판정됨
  - **자체 발견 결함 3건**: ⓐ 경보 제목과 본문이 어긋날 수 있었습니다(판정은
    실패를 먼저 보는데 경보는 변경 사건을 먼저 봤습니다) ⓑ 라이브에서
    **"백업 버킷가"** 출력 — 라벨에 받침 유무를 가정할 수 없습니다
    ⓒ **"30.0초을 넘었습니다"** — 값 표기가 `초`/`ms`로 달라집니다. 뒤 둘은
    라벨·수치를 문자열에 끼워 넣는 방식이 만든 구조적 문제라 회귀 테스트로
    막았습니다
- 하지 않은 것: 변경 사건 자동 등록(사람이 등록해야 합니다), 미해소 요구의
  취소·만료, 운영 저장소 S3 전환(표준만 정했습니다), 백업 성능 경계의 실측
  검증(현재 소요가 225ms라 주입 테스트로만 확인), 원격 사본 자동 검증(결정
  1701-④에 따라 유보)
- 질문: ① **변경 사건 등록 주체** — 지금은 사람이 `POST /ops/drills/require`로
  등록합니다. 마이그레이션이 돌아도 자동 등록되지 않으므로 **등록을 잊으면
  규칙이 없는 것과 같습니다.** `prisma migrate deploy` 시 `db-major-change`를
  자동 등록할지, 사람의 판단으로 남길지(자동화하면 사소한 마이그레이션까지
  리허설을 부르게 됩니다) ② **미해소 요구의 취소·만료** — 잘못 등록한 사건을
  되돌릴 수단이 없고, 만료도 없어 리허설 전까지 경보가 남습니다. 의도한
  동작이지만 취소 API가 필요한지 판단 요청 ③ **운영 저장소 S3 전환 시점** —
  표준은 정해졌으나 전환 전까지 보호 상태는 `직접 확인`으로 남습니다. 전환
  일정을 알려 주시면 그에 맞춰 판정·문서를 정리하겠습니다 ④ **백업 성능 경계의
  실측** — 현재 소요가 225ms라 2초·10초·30초를 실제로 넘겨 본 적이 없습니다.
  데이터가 커진 뒤 재확인이 필요한지 ⑤ **Sprint 19 진행 방향** 및 다음 TASK
  지정 요청.
