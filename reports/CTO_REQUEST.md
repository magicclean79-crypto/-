...
### 53. TASK-2001 "Enterprise Backup Integrity Platform" 세부 해석 확인
- 현황: CTO 결정 1901-①~⑤가 이번 TASK의 범위였습니다. 주제는 **남지 않은
  것을 보는 것**입니다. 지금까지의 판정은 전부 **남은 기록**을 읽었습니다 —
  백업이 실패하면 이력에 남습니다. 그런데 **돌지 않은 백업은 아무 데도
  기록되지 않습니다.** 서버가 멈춰 있던 구간에는 실패 기록조차 없어서, 이력만
  보면 **개별 백업이 전부 성공**으로 보입니다:
  - **백업 사슬 연속성**(신규 — 관측 창(24시간) 안에 **있어야 할 개수와 실제
    개수를 비교**합니다. 창 시작~첫 백업 / 백업 사이 / 마지막 백업~지금을 모두
    공백으로 세고, 공백이 **간격의 2배**를 넘으면 실패입니다. `backup-chain`은
    **복구 필수 항목**입니다 — 공백 구간은 되돌아갈 수 없어, 그 구간에 한해
    백업이 아예 없는 것과 같습니다. **한 번 걸러도 2배 안이면 통과**합니다 —
    지연에 매번 경보하면 정작 불러야 할 때 오지 않습니다)
  - **원격 사본 무결성**(결정 1701-④ 후속 — 올렸다는 **기록**과 사본이 실제로
    **있다**는 것은 다릅니다. `POST /ops/backup/verify-remote`가 내려받아
    SHA-256을 대조합니다. **전송 비용이 들어 기본으로 돌지 않으며**(결정
    1301-⑤의 연장), 확인하지 않은 상태는 통과가 아니라 `직접 확인`입니다.
    로컬 덤프로 복구는 되므로 **복구 필수 항목은 아닙니다**)
  - **운영 저장소 표준 = Amazon S3**(결정 1901-③ — 1801-④에서 기동 시
    **경고**였던 것을 체크리스트 항목 `storage-standard`로 **승격**했습니다.
    운영에서 비-S3·미설정은 **실패**입니다. 다만 **개발에서는 개발 저장소가
    정상**입니다 — 개발자에게 S3를 요구하면 비용이 들고 결국 아무도 지키지
    않습니다)
  - **DB 규모 재평가 신호**(결정 1901-④ — `pg_database_size` 실측으로
    10·50·100GB 도달을 알립니다. **크기 자체가 문제가 아니라 성능 기준을
    실측으로 다시 재라는 신호**입니다. 임계별로 경보 키가 달라 10GB를 해소하고
    50GB에서 다시 알립니다. **기준을 자동으로 바꾸지 않습니다**)
  - **Major Migration만 자동 등록**(결정 1901-① — 컬럼 하나 추가할 때마다
    리허설을 요구하면 **부를 일이 아닌데 부르는 것**입니다.
    `apps/api/prisma/major-migrations.json`에 지정된 것만 기동 시
    `_prisma_migrations`와 대조해 등록하고, `dr-change`·`pitr-adoption`은
    코드가 알 수 없는 사건이라 운영자가 직접 등록합니다)
  - **중복 등록 금지 · 삭제 금지**(결정 1901-⑤·② — 같은 trigger가 미해소면
    `409`입니다. 취소는 `cancelledBy`·`reason`이 **필수**이고, 취소된 요구는
    사라지지 않고 **사유와 함께 남습니다.** 지운 요구는 왜 지웠는지 남지
    않지만 취소는 남습니다)
  - 라이브(**실 PostgreSQL + 실 Redis + 실 S3**): 기동 시 Major Migration
    자동 등록 로그 확인 · 중복 등록 **409** · 사유 없는 취소 **400** · 취소 후
    재등록 **201** · 해소된 요구 취소 **409** · 없는 요구 **404** · 원격 사본을
    실제로 내려받아 SHA-256 일치 · `pg_database_size` 실측 10.5MB · 운영 모드
    localhost 엔드포인트가 **실패** 판정 · **실제 18.6시간 사슬 공백을 탐지해
    CRITICAL 발생 후 사슬을 메우자 RESOLVED**
  - **자체 발견 결함 3건 — 모두 라이브에서만 드러났습니다**: ⓐ **재기동이 사슬
    공백을 지웠습니다** — 관측 창을 기동 시각으로 자르는 바람에, 꺼져 있던 동안의
    진짜 공백이 사라졌습니다. 하필 사슬 판정이 잡아야 할 바로 그 상황입니다
    ⓑ **개수 제한이 없는 공백을 만들었습니다** — 최근 500건만 읽어, 창 안 4320건
    중 500건만 보이자 **21시간짜리 가짜 공백**을 보고했습니다. 개수가 아니라
    기간으로 조회하도록 고쳤습니다. 단위 테스트가 구조적으로 잡을 수 없던
    결함이라(스텁이 `where`/`take`를 무시했습니다) 스텁부터 고쳤습니다
    ⓒ **단위 표기가 사실을 가렸습니다** — 10.5MB가 "0.0GB", 20초 공백이 "0분"으로
    표시돼 *비어 있다 / 공백이 없다*로 읽혔습니다. 셋 다 회귀 테스트를 추가했습니다
- 하지 않은 것: 사슬 관측 창의 환경변수화(24시간 고정), 원격 사본 대조의
  주기 실행(비용), Major Migration 지정의 자동화(사람이 매니페스트에 넣습니다),
  운영 저장소 S3 전환(표준만 승격했습니다), 10GB 규모 실측(현재 10.5MB)
- 질문: ① **사슬 관측 창** — 24시간 고정입니다. 백업 간격이 길어지면(예: 일
  1회) 창 안 표본이 1~2개뿐이라 판정이 거칠어집니다. 창을 환경변수로 열지,
  간격에 비례해 자동 산출할지 ② **원격 사본 대조 주기** — 비용 때문에 수동으로
  두었으나, 아무도 누르지 않으면 영영 `직접 확인`으로 남습니다. 감수할 만한
  주기가 있는지(예: 주 1회, 마지막 1건만) ③ **Major Migration 지정 주체** —
  지금은 사람이 매니페스트에 넣습니다. 넣는 것을 잊으면 자동 등록도 일어나지
  않아, 자동화의 시작점이 여전히 수동입니다. 마이그레이션 파일명 규약(예:
  `_major_` 접미사)으로 대신할지 ④ **자동 등록 시점** — 기동 시 1회만
  일어납니다. 마이그레이션만 적용하고 재기동하지 않으면 등록되지 않습니다.
  이대로 둘지 ⑤ **운영 저장소 S3 전환 일정** — 이제 체크리스트에서 **실패**로
  상시 표시됩니다. 기한을 알려 주시면 그에 맞춰 정리하겠습니다 ⑥ **Sprint 20
