...
### 42. TASK-1102 "Experiment Analytics & Recommendation" 세부 해석 확인
- 현황: 지시 7항목과 **CTO 결정 1101-④·⑤**를 다음과 같이 구현했습니다.
  승격은 운영자 수동 절차라는 결정(1101-③)에 따라, 분석은 **결정을 대신하지
  않고 근거를 만든다**는 방향으로 설계했습니다:
  - **Variant Performance**: 호출·성공/실패·성공률·평균 지연·비용·호출당
    비용·토큰. 성공률에 **Wilson 95% 신뢰구간**을 붙여 "3/3 성공 = 100%"처럼
    적은 표본을 확정처럼 보이지 않게 했습니다. 근거는 배정이 아니라
    **실행(Execution)** 이고(결정 1003-④), 지연은 호출 수 **가중 평균**입니다
  - **비교**: 가중치가 가장 큰 변형을 기준(baseline)으로 성공률 %p·지연 ms·
    호출당 비용 USD 차이와 성공률 차이의 신뢰도를 제공합니다
  - **Winner Recommendation**: ① 전 변형 **최소 30회** 미달이면 추천 보류
    ② 성공률 1·2위 차이 신뢰도 **95% 이상**이면 1위 승자(확정)
    ③ 성공률 동률이면 **호출당 비용** ④ 비용도 동률이면 **지연**
    ⑤ 어느 축도 차이 없으면 보류. **품질 우선** — 싸도 실패하는 변형은
    이기지 못합니다. 성공률 근거만 `conclusive`이고 비용·지연은 참고입니다
  - **Confidence Score**: 양측 2-비율 z검정(core 순수 함수 `normalCdf`
    (erf 근사)·`wilsonInterval`·`proportionConfidence`)
  - **Analytics Dashboard**: `GET /llm/experiments/:feature/analytics` + 웹
    `/experiments` 카드 내 추천 배지(확정/참고 구분)·근거 문장·비교표
  - **결정 1101-④ 이행**: Start/Stop = EDITOR 이상, **Promote/Rollback =
    ADMIN 전용**. Express 5의 path-to-regexp가 `:action(start|stop)` 문법을
    지원하지 않아 **4개 명시 라우트**로 분리했습니다(권한 의도가 라우트에
    그대로 드러나 오히려 명확해졌습니다)
  - **결정 1101-⑤ 이행**: 정의 변경 재배정을 `experiment_assignment_events`에
    기록(프로젝트·이전/이후 변형·이전/이후 서명·사유). **최초 배정은 재배정이
    아니므로 기록하지 않습니다.** Assignment Dashboard에 표시됩니다
  - 라이브: 표본 1·3회에서 추천 보류(Wilson 0.21~1.00으로 과신 방지) →
    34·102회 축적 후 **신뢰도 1.0000 확정 추천(mock)**, 정의 변경 재배정
    Audit 2건, EDITOR promote/rollback **403** · ADMIN **200**
- **운영상 걸리는 점**: 관측 기간을 **마지막 START 이후**로 잡았기 때문에
  STOP→START를 하면 그전에 모은 근거가 분석에서 빠집니다. 라이브 검증 중
  권한 테스트로 START를 호출하자 관측이 0건이 되는 것을 실제로 겪었습니다.
  "실험을 다시 시작하면 근거도 새로 모은다"는 의도였으나, 잠시 멈췄다 재개한
  경우에는 불편할 수 있어 확인이 필요합니다.
- 하지 않은 것: 다중 비교 보정(변형 3개 이상일 때 1·2위만 검정), 기간 지정
  분석(현재 관측 시작 이후 전체), 승격 자동화(결정 1101-③에 따름),
  실험 정의의 ADMIN 화면 관리
- 질문: ① **최소 표본 30회**가 적절한지 (더 엄격하게 할지, 환경변수로 조정
  가능하게 할지) ② **판단 순서** — 성공률 → 비용 → 지연이 맞는지, 비용을
  더 앞에 두어야 하는 영역이 있는지 ③ **관측 기간 초기화** — 위 사항에서
  현재처럼 START마다 초기화할지, 아니면 정의(서명)가 바뀔 때만 초기화하고
  START/STOP은 기간을 이어갈지 ④ **다중 비교 보정** — 변형 3개 이상 실험을
  운영할 계획이면 보정(Bonferroni 등) 도입 여부 ⑤ 다음 TASK 지정 요청.
