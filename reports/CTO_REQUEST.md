...
### 43. TASK-1201 "Provider Administration Console" 세부 해석 확인
- 현황: 지시 5항목과 **CTO 결정 1102-①·③**을 다음과 같이 구현했습니다.
  설계의 핵심 판단은 **Code-first 원칙을 깨지 않은 것**입니다 — 그동안 예산
  (0902-③)·실험 정의(1003-②)를 환경변수로 확정해 왔으므로, 콘솔을 새로운
  진실 공급원으로 만들지 않고 **그 위에 얹는 오버라이드**로 두었습니다:
  - `유효값 = 오버라이드(DB) ?? 환경변수 ?? 기본값`. 오버라이드가 없으면
    **기존 동작과 완전히 동일**하고, 해제하면 환경변수로 되돌아갑니다.
    화면은 각 값의 **출처(콘솔/환경변수/기본값)** 와 복귀값을 함께 보여줍니다
  - **Provider Enable/Disable**(`provider.<name>.enabled`) — 끄면 라우팅·실험·
    Failover 후보에서 제외. **마지막 하나는 끌 수 없습니다**(호출 전멸 방지)
  - **Model Management**(`model.<feature>`) · **Budget Management**
    (`budget.daily`/`monthly`/`alertRatio`) · **Experiment Management**
    (`experiment.<feature>`) — 각각 대응 환경변수보다 우선
  - **Audit Log** — 누가·무엇을·이전→이후·시각. 모든 변경이 기록됩니다
  - **검증은 저장 시점**에 합니다(알 수 없는 Provider·음수 예산·해석 불가한
    실험 정의·지원하지 않는 키 → 400). 잘못된 값이 저장되면 다음 호출부터
    라우팅이 깨지기 때문입니다
  - **읽기는 동기**입니다 — 설정은 LLM 호출마다 참조되므로 DB 왕복을 넣지
    않고, 기동 시 인메모리 적재 + 쓰기 시 즉시 갱신 + TTL(기본 10초) 경과 시
    백그라운드 재적재로 처리했습니다
  - **전부 ADMIN 전용**이며, 결정 1102-①(최소 표본 `LLM_EXPERIMENT_MIN_SAMPLES`)
    과 ③(관측 기간은 **Definition Signature 변경 시에만** 초기화,
    START/STOP은 기간 유지)을 함께 이행했습니다
  - 라이브: 모델 오버라이드 → 라우팅 즉시 반영 · openai 비활성 → 후보 축소 ·
    예산 10 → 99 반영 · 실험 정의 콘솔 설정 · 가드레일 4종 400 ·
    EDITOR 403 · 미인증 401
- **라이브 검증 중 발견해 고친 보안 결함**: `GET /admin/console`과
  `/admin/audit`이 **미인증 200**으로 열려 있었습니다. 전역
  WriteProtectionGuard가 쓰기만 막고 조회는 기존 정책상 비보호이기 때문인데,
  콘솔의 GET은 예산·모델 구성과 **감사 이력의 수행자 이메일**을 노출합니다.
  컨트롤러에 AuthGuard를 붙여 조회까지 ADMIN 전용으로 만들고(`/llm/health`가
  같은 이유로 별도 가드를 쓴 선례) 회귀 테스트를 남겼습니다. 아울러
  `LlmBudgetService`가 모듈에서 export되지 않아 콘솔이 기동하지 않던 문제도
  라이브에서 드러나 수정했습니다(단위 테스트로는 잡히지 않는 배선 결함).
- 하지 않은 것: 라우팅 규칙(`LLM_ROUTE_*`)의 콘솔 편입(지시 5항목에 없음),
  설정 변경 알림, 오버라이드 일괄 정리, 다중 인스턴스 즉시 전파
- 질문: ① **오버라이드 계층 방식**이 맞는지 — Code-first를 지키려고 환경변수를
  기본값으로 남겼습니다. 반대로 "콘솔이 진실 공급원이고 환경변수는 초기값"이
  맞다면 구조를 바꿔야 합니다 ② **다중 인스턴스 전파** — 조회를 동기로
  유지하려고 TTL(기본 10초) 지연을 택했습니다. 즉시 전파가 필요하면 Redis
  pub/sub 등을 도입해야 하는데, 현재 규모에서 필요한지 ③ **라우팅 규칙
  (`LLM_ROUTE_*`)** 도 콘솔에 넣을지 (현재는 Provider 활성/모델/실험만)
  ④ **배포와 오버라이드의 관계** — 배포로 환경변수를 바꿔도 오버라이드가 남아
  있으면 계속 우선합니다. "배포 시 오버라이드 정리" 같은 운영 규칙이 필요한지
  ⑤ 다음 TASK 지정 요청.
