...
### 50. TASK-1701 "Enterprise Backup & Disaster Recovery" 세부 해석 확인
- 현황: TASK-1601이 "백업이 있고 복원된다"를 증명했다면, 이번 주기는 그 다음
  세 질문이었습니다 — **덤프가 온전한가 · 같은 곳에만 있지 않은가 · 얼마를
  잃고 얼마나 걸리는가**:
  - **Backup Integrity**: 백업 직후 `pg_restore --list`로 목차를 읽고 SHA-256을
    기록합니다. **복원하지 않고도 파일이 온전한지 알 수 있는 가장 싼 검사**
    입니다. 읽히지 않는 덤프는 존재하지만 쓸 수 없는 파일이고, 그것을 "백업
    있음"으로 세면 복구 계획이 통째로 거짓이 됩니다. 무결성 실패는 **복구
    가능성을 좌우하는 항목**으로 두었습니다. 라이브에서 **2.4KB 쓰레기 파일을
    최신 덤프로 만들자 크기 검사는 통과했지만 `복구 불가`로 판정**됐습니다
  - **Offsite Replication**(`BACKUP_OFFSITE=on`): 덤프를 오브젝트 저장소에도
    올립니다. **복제 실패가 백업을 실패시키지는 않습니다**(덤프는 이미
    받았습니다) — 대신 로그로 남기고 화면에서 드러냅니다. 미구성은 실패가 아니라
    주의이고 복구 가능성을 막지도 않습니다(로컬 덤프로 복구는 됩니다). 원격 키는
    노출하지 않고 **있는지만** 알립니다. 라이브에서 **원격 사본의 SHA-256이
    기록된 체크섬과 정확히 일치**함을 확인했습니다
  - **RPO·RTO**: RPO는 마지막 성공 백업 이후 경과, RTO는 **마지막 복원 검증에
    실제로 걸린 시간**입니다 — 추정이 아니라 측정입니다(복원 검증이 이미 매일
    돌고 소요 시간을 기록하고 있어 별도 계측이 필요 없었습니다). 다만 **하한**
    이며 장애를 알아차리고 결정하고 인프라를 준비하는 시간은 포함하지 않습니다 —
    화면에도 그렇게 적었습니다. 측정치가 없으면 `직접 확인`이고 통과가 아닙니다
  - **복원 대상 강제 분리**(결정 1601-②): 호스트·포트·DB 이름만 비교해
    **자격 증명이 달라도 같은 DB는 같은 DB**로 보고, 같으면 **기동하지
    않습니다**. 개발 환경에서도 오류로 두었습니다 — 운영 DB를 지우는 설정은
    어디서도 허용할 수 없다고 판단했습니다
  - **BACKUP_DIR 운영 필수**(결정 1601-①): 미설정이면 **기동하지 않습니다**.
    라이브에서 두 Fail Fast 모두 실제로 기동을 막는 것을 확인했습니다
  - **이미지 저장소 보호**(결정 1601-④): 버킷의 Versioning·Replication을 읽어
    체크리스트에 표시합니다. **애플리케이션은 이미지를 직접 백업하지 않습니다.**
    저장소가 알려 주지 않으면 `직접 확인`으로 남깁니다 — 조회를 지원하지 않는
    저장소를 "정상"이라고 말하면 그 화면은 거짓이 됩니다. 꺼져 있으면 주의이지
    실패가 아닙니다(버전 관리는 삭제·덮어쓰기로부터 보호하는 장치이지, 지금
    이미지가 없다는 뜻이 아닙니다)
  - **WAL·PITR·Incremental 검토**(결정 1601-③): 구현하지 않고
    `docs/operations/backup-strategy.md`로 판단 근거를 정리했습니다. 요약하면
    **백업 주기 단축을 먼저 권고**합니다 — RPO 24시간→1시간을 **복구 절차 변경
    없이** 얻을 수 있고, 덤프가 100KB라 하루 24회를 받아도 2.4MB입니다. PITR은
    RPO를 분 단위로 줄이지만 **복구를 더 어렵게** 만들어(지금은 `pg_restore` 한
    줄, PITR은 기준 백업 선택 + WAL 재생), **연습하지 않은 PITR은 없는 것보다
    위험합니다**. 증분은 PostgreSQL 17이 필요하고(현재 16) 100KB 규모에서 아낄
    것이 없습니다. 재검토 조건 4가지를 전부 이미 기록 중인 값(`backup_runs`)으로
    확인할 수 있게 두었습니다
  - **신선도 기본값**(결정 1601-⑤): 백업 2일·복원 검증 8일을 유지하고, 기본값을
    회귀 테스트로 고정했습니다
  - **자체 발견 결함 1건**: 환경변수 설명의 마크다운 `**`가 기동 실패 로그에
    별표째 출력됐습니다. 1302·1601에 이은 **같은 계열 세 번째 재발**이라, 이번엔
    개별 문구가 아니라 **선언 목록 전체를 훑는 회귀 테스트**로 막았습니다
- 하지 않은 것: 원격 사본의 자동 무결성 검증(체크섬을 기록해 사람이 대조할 수는
  있으나 예약 점검이 내려받아 확인하지는 않습니다), 백업 주기 단축(권고만 하고
  결정을 기다립니다), 복원 후 행 수·제약 검증(테이블 수까지만), 실 Provider
  스모크(샌드박스 차단 + 과금)
- 질문: ① **백업 주기 단축 채택 여부** — `backup-strategy.md` §3의 권고입니다.
  RPO 24시간을 1시간으로 줄이는 데 **복구 절차 변경이 필요 없고** 비용도 사실상
  없습니다. 채택할지, 지금의 하루 1회를 유지할지 ② **원격 복제 대상** — 지금은
  이미지와 **같은 버킷**에 접두사(`backups/`)로만 나눠 둡니다. 같은 버킷이
  사라지면 이미지와 백업이 함께 사라지므로 원격 복제의 목적을 절반만
  달성합니다. 별도 버킷/계정/리전을 쓸지, 쓴다면 어느 수준까지 분리할지
  ③ **저장소 보호 조회** — MinIO·s3rver는 복제 조회를 지원하지 않아 `직접 확인`
  으로 남습니다. 실제 운영 저장소(제공자 미정)에서 어떻게 나오는지 확인하지
  못했습니다. 운영 저장소 제공자를 알려 주시면 그에 맞춰 판정을 다듬겠습니다
  ④ **원격 사본 자동 검증** — 예약 점검이 원격 사본을 내려받아 체크섬을
  대조하도록 확장할지(전송 비용이 듭니다) ⑤ **복구 리허설 절차화** — 분기 1회
  실제 복구를 운영 절차로 넣을지. 이는 **PITR 도입의 선행 조건**이기도 합니다
  ⑥ **Sprint 17 진행 방향** 및 다음 TASK 지정 요청.
