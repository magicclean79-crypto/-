...
| 태그 | **`v1.0.0-rc.1`** (CTO 승인). 정식 `v1.0.0`은 **붙이지 않았습니다** — 조건이 "Validation 통과 시"이고 `validation_runs`는 **0건**입니다 |
...
`v1.0.0-rc.1` 태그가 가리키는 커밋에서 워크플로 3 Job이 모두 통과했습니다.
실행 번호와 소요는 §2 표를 갱신하며 기록했습니다.
