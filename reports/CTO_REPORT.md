...
| ~~B-8~~ | ~~KPI 기준선 1점~~ | ✅ **해제** (§4-8) — 24.0시간 뒤 2점째 확보 |
...
| 항목 | 필요한 것 | 예상 |
| --- | --- | --- |
| `backup-chain` 17.9시간 공백 | 호스트 **24시간 연속 가동** + `OPS_SCHEDULED_CHECKS` | 24시간 — **staging에서** |
| ~~KPI 기준선 2점~~ | ✅ **해제** (§4-8) | — |
| 비용 귀속률 목표 | 귀속 대상 호출 **20건 이상** | 자격 증명 이후 |

`backup-chain`은 **로컬에서 채우지 않기로 했습니다.** 로컬 머신을 24시간 켜
두어도 출시에는 기여하지 않습니다 — 이 항목은 staging 호스트에서
`OPS_SCHEDULED_CHECKS=on`으로 채워져야 합니다.
