...
#### staging 판정 — S3 리전 결함 수정 후 (2026-08-06 최신)

**실 AWS 자격 증명을 연결하고 처음으로 `storage`가 `pass`가 됐습니다.**
원인은 자격 증명이 아니라 **제품 코드 결함**이었습니다 — minio 클라이언트가
리전을 명시하지 않아 `ap-northeast-2`에서 서명이 거부됐습니다
(`1323127` · §4-7 형식의 새 발견, DoD 기록은 `reports/CTO_REPORT.md` 참고).

| 판정 | 조치 전 | **조치 후(현재)** |
| --- | --- | --- |
| `/ops/readiness` | pass 9 · fail 2 | pass **11** · fail **1**(`backup-chain`, 시간 의존) |
| 검증 계획 | 3/11 (사람 4 · 우리 2) | **5/11** (사람 **3** · 우리 **1** · 막힘 2) |
| 활성화 런북 | 1/8 | **2/8** |
| `/ops/go-live` | `not-started` 1/8 | `not-started` **2/8** |
| 운영 기동 차단 | 4건(전부 S3) | **0건** — S3 필수 4개 전부 충족 |

**"공식 주소로 나가는 길 열기"·"기동 진단 실패 0건" 두 단계도 자동으로
완료**됐습니다 — `storage`가 진단 실패 목록에 있었기 때문입니다.

남은 사람 몫 3건과 결정(2026-08-06):

| 항목 | CTO 결정 |
| --- | --- |
| 실 Provider 자격 증명 | OpenAI 키 — 전달 방법 확정, 전달 대기 |
| 운영 호스트 목록 | **아직 없음** — staging 주소만 존재. **staging 주소를 `PRODUCTION_HOSTS`에 넣지 않습니다** — 넣으면 검증 대상 보호가 스스로 무력해집니다 |
| 긴급 알림 경로 | **아직 없음** — Webhook을 나중에 생성하기로 함 |

남은 "우리" 몫 1건(KPI 기준선)은 시간이 막고 있습니다 — 첫 스냅샷
`2026-08-06T06:25:43Z`, 2점째는 **UTC 날짜가 바뀐 뒤**(`2026-08-07T00:00Z`
이후)에나 찍힙니다.
...
#### 재부팅 복구 검증 (2026-08-06)

`backup-chain`은 **인스턴스가 24시간 연속 살아 있어야** 해소됩니다. 그 전제가
실제로 성립하는지 재부팅으로 시험했습니다 — AWS 유지보수 재부팅 한 번에 24시간
창이 조용히 리셋되면 알아차릴 방법이 없기 때문입니다.

| 확인 | 결과 |
| --- | --- |
| SSH 복구 | **53초** |
| `docker` | 자동 기동 (`enabled`) |
| `acos-postgres` 컨테이너 | 자동 복귀 (`restart: unless-stopped`) |
| `acos-api` · `acos-web` | 둘 다 자동 기동 |
| **`acos-api` 재시작 횟수** | **0** — Docker·Postgres 기동 경쟁 없이 한 번에 성공 |
| `/health` · `/login` | 첫 확인에 **둘 다 200** |
| 스왑 4GB | `/etc/fstab` 등록으로 자동 복구 |

**재시작 횟수 0이 핵심입니다.** `acos-api`는 `After=docker.service`로만 걸려
있어 Postgres 컨테이너가 늦게 뜨면 실패 후 재시도(`Restart=on-failure`)할
수 있는 구조인데, 실제로는 재시도 없이 올라왔습니다. 다만 이것은 **이번 한 번의
관측**이며, 부팅이 느린 날에는 재시도가 일어날 수 있습니다 — 그때도 5초 뒤
자동 복구됩니다.
